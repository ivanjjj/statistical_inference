...
sd(x)
```
We can see that the standard deviation is close to the theoretical standard deviation of 5 (1/lambda(0.2)) Let's also take a look at mean standard deviation accross all the samples
```{r test exponential variance}
mean(apply(x,1,sd))
```
We can also see here that this is close to the theoretical standard deviation of 5.
